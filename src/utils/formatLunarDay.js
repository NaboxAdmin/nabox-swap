...
function GetLunarDay(solarYear, solarMonth, solarDay) {
    ...
    if (solarYear < 1901 || solarYear > 2999) {
        return ''
    } else {
...
export const lunarCalendar = (str) => {
    const D = new Date(str)
    let yy = D.getFullYear()
    const mm = D.getMonth() + 1
    const dd = D.getDate()
    if (yy < 100) yy = '19' + yy
    const data = GetLunarDay(yy, mm, dd)
    return data.substr(data.length - 2, 2)==='初一'?data.substr(0, 2):data.substr(data.length - 2, 2)
}
